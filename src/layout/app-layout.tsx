...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['trader', 'analyst'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BrokerageInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/brokerages` : null),
    () => getBrokerages({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Brokerage',
      path: isTenantUser ? `/brokerages/view/${data?.[0]?.id}` : '/brokerages',
      entity: 'brokerage',
